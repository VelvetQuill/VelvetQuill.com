...
class ApiService {
    ...
    const isPublicEndpoint = endpoint.includes('/public/') || 
                            ...
                            endpoint.includes('/stories/search') ||
    ...
    try {
        ...
        // For public endpoints, don't trigger auth redirects on 401 errors
        if (error.status === 401 && isPublicEndpoint) {
            // Return a normalized error that doesn't trigger redirects
            throw new Error('Public content not available');
        }
    ...
    buildRequestConfig(options = {}, isPublicEndpoint = false) {
        const config = {
    ...
    // Only redirect if not already on a public page and not on auth pages
    const currentPath = window.location.pathname;
    const isPublicPage = currentPath.includes('author-room') || 
                        currentPath.includes('story-read') ||
                        currentPath.includes('index');
    const isAuthPage = currentPath.includes('signin') || 
                      currentPath.includes('signup') || 
                      currentPath.includes('login');
    
    // Don't redirect for public pages - let them continue without auth
...
/**
 * Get public stories by author (no authentication required)
 * @param {string} identifier - Author username or ID
 * @param {Object} options - Query options
 * @param {number} options.page - Page number
 * @param {number} options.limit - Items per page
 * @param {string} options.sortBy - Sort field
 ...
 * @returns {Promise<Object>} Response with stories and author info
 */
async getPublicStoriesByAuthor(identifier, options = {}) {
    const { 
        page = 1, 
        limit = 6, 
        sortBy = 'createdAt', 
 ...
 * @param {string} authorIdentifier - Author username or ID being viewed
 * @returns {boolean} True if current user is viewing their own profile
 */
isViewingOwnProfile(authorIdentifier) {
    const currentUser = AuthManager.getCurrentUser();
    if (!currentUser) return false;
    
    // Check if identifier matches current user's username or ID
    return authorIdentifier === currentUser.username || 
           authorIdentifier === currentUser.id ||
           authorIdentifier === currentUser._id;
}
